...
for result in results:
    ...
    np.save(os.path.join(BASE_DIR, "./data/yolo-seg/", image_name[:-4]+".npy"), mask)
    ...
    if image_index==max_col*max_row:
        plt.tight_layout()
        plt.savefig(os.path.join(BASE_DIR, f"./data/yolo-seg/display/pred_yolo-seg_{window_index}.png"), dpi=300)
        image_index = 0
        window_index+=1
        fig, axes = plt.subplots(4, 4, figsize=(5 * max_col, 5 * max_row))
        axes = axes.flatten()
