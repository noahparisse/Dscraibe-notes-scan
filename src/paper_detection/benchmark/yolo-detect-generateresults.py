...
from src.paper_detection.yolo.segmentation_threshold import get_mask
import cv2
...
model = YOLO(os.path.join(BASE_DIR, "../yolo/model/best-detect.pt"))
...
for result in results:
    ...
    for i in range(result.boxes.shape[0]):
        ...
        x, y, w, h = rect
        roi = result.orig_img[y:y+h, x:x+w]
        mask = get_mask(roi)
        entire_mask = np.zeros(result.orig_img.shape[:2], np.uint8)
        entire_mask[box_y_top:box_y_top + height, box_x_left:box_x_left + width] = mask
        mask_sum += entire_mask
    ...
    mask_color[mask_sum > 0] = [0, 0, 255]  # Red in BGR (so it appears blue in RGB when using cv2.imshow)
